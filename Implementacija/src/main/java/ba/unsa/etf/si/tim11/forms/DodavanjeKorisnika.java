...
public class DodavanjeKorisnika {
	...
	private Boolean uredu[] = { false, false, false, false, false, false };// Niz
																			// za
																			// provjeru
																			// validnosti
																			// svakog
	...
	private void initialize() {
		...
		textFieldDodavanjeKorisnikaUsername.addFocusListener(new FocusAdapter() {
			...
			public void focusLost(FocusEvent e) {
				...
				if (!Validator.daLiJeStringPrazan(s) && Validator.daLiJeStringSlovaIBrojevi(s)
						&& kr.dajIdKorisnikaPoUsername(s) == null) {
					labelOkUsername.setText("OK");
					labelOkUsername.setForeground(new Color(0, 128, 0));
					uredu[3] = true;
				} else if (kr.dajIdKorisnikaPoUsername(s) != null) {
					JOptionPane.showMessageDialog(frmDodavanjeizmjenaKorisnika,
		...
		btnDodavanjeSpremi.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					Integer korisnikTipId = (int) ((KorisnikTipDbModel) comboBoxDodavanjeTip.getSelectedItem())
							.getKorisnikTipId();
					Integer korisnikPozicijaId = (int) ((KorisnikPozicijaDbModel) cmbDodavanjePozicija
							.getSelectedItem()).getKorisnikPozicijaId();
